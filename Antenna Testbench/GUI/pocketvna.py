...
def priv_load_local_library_(orig_fn):
    global PVNA
    pocketvna_path = os.path.dirname(os.path.abspath(__file__))
    path = pocketvna_path + '/libPocketVnaApi.so'
    
    if path is None:
        path = priv_find_file_(priv_compound(BaseName), pocketvna_path)
    if path is None:
        raise
    PVNA = CDLL(path)
    
    print('Local pocketVNA library ' + path + ' is loaded')
    return PVNA
...
pocketvna = priv_load_local_library_(priv_get_lib_name_())
